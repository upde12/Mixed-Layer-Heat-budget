# 6) 과학적 글쓰기·발표·토론 지침
<!-- owner: MLHB-core; canonical: true; depends_on: docs/guidelines/01_llm_guidelines.md; last_review: 2025-09-29 -->
...
## B‑1. 주장·방법에 대한 문헌 인용(의무)
- 범위: 방법 선택(예: ADV vs flux form, TEN 차분, SHALLOW/연안 마스킹, fully‑mixed에서 ENT/DIFFV=0), 정책 결정(운영 기본값), 해석(결론·권고).
- 의무: 각 주장 유형별로 대표 선행연구 2–3편 이상을 본문(배경/방법 끝)과 그림 캡션(RefID 간단 표기)에 병기한다.
- 매핑 표: “방법↔참고문헌 매핑 표”를 본문에 포함한다(필드: Method | RefID | Dataset/Scope | Where used | Notes). RefID는 `references/<주제>/overview.md` 또는 `_extracted/*.md`의 항목 키를 사용한다.
- 내부 레퍼런스: 서지/요약/적용 범위/한계는 `references/<주제>/` 트리(지침 07)에서 관리하고, 보고서에서는 RefID를 호출한다.

### QA 보강
- [ ] 각 주장에 대해 2–3편 선행연구가 본문·캡션에 RefID로 병기되었는가?
- [ ] 방법↔참고문헌 매핑 표가 포함되었는가?
...
## E. 참고문헌·요약 관리
- 본 섹션은 분리되었습니다. 참고문헌 보관/이름 규칙/요약본 관리에 대한 지침은 `docs/guidelines/07_reference_management.md`를 참조하세요.
...
## G. 언어 사용 규칙
- 본 섹션은 발표·슬라이드 맥락의 언어 규칙에 한정한다. 일반 대화/운영 중 언어 규칙은 `docs/guidelines/01_llm_guidelines.md`를 따른다.

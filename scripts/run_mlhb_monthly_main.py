#!/usr/bin/env python3
"""Run MLHB monthly pipeline using the mainline processor (centered, forward)."""
...
def run_daily(window: MonthWindow, args) -> Path:
    ...
    # Allow overriding start/end date for edge cases (e.g., partial months)
    start_date = getattr(args, "override_start_date", None) or window.start.isoformat()
    end_date = getattr(args, "override_end_date", None) or window.end.isoformat()

    cmd = [
        args.python,
        "src/process_d2nf_main.py",
        "--indir",
        args.indir,
        "--outdir",
        str(outdir),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run MLHB monthly pipeline (mainline)")
    ...
    parser.add_argument("--out", default="/Volumes/HJPARK4/Decadal/source/ML_budget/output/monthly/mlhb_monthly_main.nc")
    ...
    # Optional per-run overrides for start/end-date (YYYY-MM-DD). Useful for partial months.
    parser.add_argument("--override-start-date", default=None)
    parser.add_argument("--override-end-date", default=None)
